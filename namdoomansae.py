...
# 카페24 가상환경에서는
swe.set_ephe_path('/root/.local/share/virtualenvs/mansae-zYGbz4Ou/lib/python3.8/site-packages/flatlib/resources/swefiles')
...
with tab1:  # 만세력 탭
    ...
    with col2:
        ...
        for i in range(1, j + 1):
            ...
            # 황경값에 대한 딕셔너리
            HwangGyung = {0: '춘분', 15: '청명', 30: '곡우', 45: '입하', 60: '소만', 75: '망종', 90: '하지', 105: '소서', 120: '대서',
                          135: '입추',
                          150: '처서', 165: '백로', 180: '추분', 195: '한로', 210: '상강', 225: '입동', 240: '소설', 255: '대설',
                          270: '동지',
            ...
            for k in HwangGyung:
                JD_return = swe._next_aspect(swe.SUN, 0, k, JD_today - K_tzoffset_day,
                                             dayspan=1)  # 태양이 절기의 황경의 값에 있을때 줄리안 데이트를 리턴
                if bool(JD_return[0][0]) != bool(''):
                    ...
                    JeulGi = str(HwangGyung[k]) + ' ' + str(JeulGiIl) + '일' + str(JeulGiSi) + '시' + str(
                        JeulGiBun) + '분' + str(JeulGiCho) + '초'
                    break
        ...
        # AgGrid(data=df2, columns_auto_size_mode=ColumnsAutoSizeMode.FIT_CONTENTS,enable_enterprise_modules=False)
        AgGrid(df2, gridOptions=go, columns_auto_size_mode=ColumnsAutoSizeMode.FIT_CONTENTS,
